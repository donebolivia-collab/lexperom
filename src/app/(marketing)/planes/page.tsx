  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-20">
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-3">
        {legalPlans.map((plan) => (
          <Card
            ...
            className={cn(
              "flex flex-col",
              plan.highlight && "border-2 border-header-accent shadow-md"
            )}
          >
              ...
              <ul className="mt-6 flex-1 space-y-3">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2 text-sm text-ink-soft">
                    <Check className="mt-0.5 h-4 w-4 shrink-0 text-header-accent" aria-hidden="true" />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
      ...
      <div className="mx-auto mt-12 max-w-2xl space-y-4 text-sm leading-relaxed text-muted">
        <p>
          <strong className="font-medium text-ink-soft">¿Esto es un seguro?</strong> No. Es una
          membresía de acceso directo a servicios legales — como una revisión médica preventiva,
          pero para tus temas legales. No sustituye ni funciona como una póliza de seguro.
        </p>
        <p>
